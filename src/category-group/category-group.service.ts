...
export class CategoryGroupService {
    ...
    async create(id:number,data: CreateCategoryGroupDto):Promise<CategoriesGroup>{
        return this.prismaService.categoriesGroup.create({data:{...data,user_id:id}})
    }
    async getDetail(id:number):Promise<CategoriesGroup>{
        return await this.prismaService.categoriesGroup.findUnique({where: {id}})
    }
    async getAll(filters: CategoryGroupFilterType): Promise<CategoryGroupPaginationResponseType> {
    ...
    async getAllTrash(filters: CategoryGroupFilterType): Promise<CategoryGroupPaginationResponseType> {
        const items_per_page = Number(filters.items_per_page) || 10;
        const page = Number(filters.page) || 1;
        const search = filters.search || '';
        const skip = page > 1 ? (page - 1) * items_per_page : 0;
    ...
    async getAllForUser(id: number,filters: CategoryGroupFilterType): Promise<CategoryGroupPaginationResponseType> {
        const items_per_page = Number(filters.items_per_page) || 10;
        const page = Number(filters.page) || 1;
        const search = filters.search || '';
        const skip = page > 1 ? (page - 1) * items_per_page : 0;
    ...
    async update(id: number, data: UpdateCategoryGroupDto):Promise<CategoriesGroup>{
